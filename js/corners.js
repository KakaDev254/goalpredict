...
function renderTodayCorners() {
  const container = document.getElementById("corners-container");
  const totalOddsElement = document.getElementById("corners-total");
  ...
  const todayPredictions = cornersPredictions.find((p) => p.date === today);
...
function renderPastCorners() {
  const container = document.querySelector("#corners .table-rows");
  const totalOddsElement = document.getElementById("corners-total");
  ...
  if (selectedDate) {
    datePredictions = cornersPredictions.find((p) => p.date === selectedDate);
    displayDate = selectedDate;
  } else {
    // Use the most recent date (first in array)
    datePredictions = cornersPredictions[0];
    displayDate = datePredictions ? datePredictions.date : null;
  }

  // Add date selector
  addCornersDateSelector(displayDate);
...
function addCornersDateSelector(selectedDate) {
  const section = document.getElementById("corners");
  if (!section) return;

  // Check if selector already exists
  let selectorContainer = document.getElementById("corners-date-selector");
  if (!selectorContainer) {
    selectorContainer = document.createElement("div");
    selectorContainer.id = "corners-date-selector";
  ...
  let selectorHTML = `
    <div class="d-flex align-items-center gap-2">
      <label for="corners-date-picker" class="fw-bold">Select Date:</label>
      <select id="corners-date-picker" class="form-select form-select-sm" style="width: auto;">
  `;

  // Add all available dates from the data
  cornersPredictions.forEach((prediction) => {
    const today = new Date().toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "short",
      year: "numeric",
  ...
  // Add event listener
  const datePicker = document.getElementById("corners-date-picker");
  if (datePicker) {
...
// Function to calculate total odds for corners (can be used if needed separately)
function calculateCornersTotalOdds(matches) {
